...
def nan_to_num(tensor, nan=0.0):
    """
    Replace NaN values in a tensor with a specified value (default: 0.0).
    """
...
def _normalize(tensor, dim=-1):
    """
    Normalize a tensor along a given dimension, replacing NaN with 0.
    """
    return nan_to_num(
...
def cal_dihedral(X, eps=1e-7):
    """
    Compute dihedral angles for a sequence of backbone coordinates.
    """
    dX = X[:,1:,:] - X[:,:-1,:] 
...
def _dihedrals(X, dihedral_type=0, eps=1e-7):
    """
    Compute dihedral and planar angle features for protein backbone.
    Returns concatenated cos/sin encoding of psi, omega, phi, alpha, beta, gamma.
    """
    B, N, _, _ = X.shape
    # psi, omega, phi
...
def _hbonds(X, E_idx, mask_neighbors, eps=1E-3):
    """
    Compute hydrogen bond features between residues.
    Uses a simple electrostatic energy model to identify H-bonds.
    """
...
def _rbf(D, num_rbf):
    """
    Apply radial basis function (RBF) encoding to distance tensor D.
    Maps scalar distances into a fixed-length vector of Gaussian responses.
    """
...
def _get_rbf(A, B, E_idx=None, num_rbf=16):
    """
    Compute RBF encoding of pairwise distances between point sets A and B.
    If E_idx is provided, only compute neighbor distances.
    """
...
def _orientations_coarse_gl(X, E_idx, eps=1e-6):
    """
    Compute coarse-grained orientation features between residues:
    - relative displacement vectors in local frame
    - relative rotation encoded as quaternion
    """
...
def _orientations_coarse_gl_tuple(X, E_idx, eps=1e-6):
    """
    Variant that returns tuple of (node orientation, edge direction, edge rotation).
    Used for vector features on nodes and edges.
    """
    V = X.clone()
    X = X[:,:,:3,:].reshape(X.shape[0], 3*X.shape[1], 3) 
    dX = X[:,1:,:] - X[:,:-1,:]
...
def gather_edges(edges, neighbor_idx):
    """
    Gather edge features for neighbors based on neighbor_idx.
    """
...
def gather_nodes(nodes, neighbor_idx):
    """
    Gather node features for neighbors based on neighbor_idx.
    """
...
def _quaternions(R):
    """
    Convert 3x3 rotation matrices to normalized quaternion representation.
    """
